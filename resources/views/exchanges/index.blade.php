@extends('layouts.app')
@section('title', 'دایرکتوری صرافی‌ها | آقای صرافی')
@section('body_start')
  ...
  <header class="sticky-header" id="stickyHeader">
    <div class="header-inner">
      ...
      <div class="header-toolbar converter-toolbar" id="converterToolbar" hidden>
        {{-- empty or optional converter tools --}}
      </div>
  ...
  <main class="directory-list" id="directoryList">
    <div class="exchange-cards" id="exchangeCards">
      ...
      @foreach($offices as $office)
        ...
        @endphp
        <article class="exchange-card {{ (is_array($office->features ?? null) && in_array('highlight', $office->features)) ? 'exchange-card-highlight' : '' }}" role="button" tabindex="0"
          data-id="{{ $office->id }}"
          data-name="{{ e($office->name) }}"
          data-address="{{ e($addressFull) }}"
          data-phone="{{ e($office->phone ?? '') }}"
          data-email="{{ e($office->email ?? '') }}"
          data-buy="{{ $buy }}"
          data-sell="{{ $sell }}"
          data-verified="{{ $office->identity_verified ? '1' : '0' }}"
          data-map-url="{{ $mapUrl }}"
          data-logo-url="{{ $office->logoUrl() ?? '' }}"
          data-features="{{ e(json_encode($office->features ?? [])) }}"
          data-currencies="{{ e(json_encode($office->currencies ?? [])) }}"
          data-special-rate-buy="{{ $office->special_rate_buy ?? '' }}"
          data-special-rate-sell="{{ $office->special_rate_sell ?? '' }}"
          data-payment-methods="{{ e(implode(',', $office->getAcceptedPaymentMethods())) }}"
          data-trend-buy="{{ $trend['buy'] ?? '' }}"
          data-trend-sell="{{ $trend['sell'] ?? '' }}"
          data-transfer-fee-under="{{ $office->transfer_fee_under_amount ?? '' }}"
          data-transfer-fee-amount="{{ $office->transfer_fee_amount ?? '' }}">
          <div class="card-header">
            <div class="card-brand">
              @if($office->logoUrl())
                <img src="{{ $office->logoUrl() }}" alt="" class="card-logo card-logo-img" width="40" height="40">
              @else
                <div class="card-logo" aria-hidden="true">{{ mb_substr($office->name, 0, 1) }}</div>
              @endif
              <span class="card-name">{{ $office->name }}
                @if($office->identity_verified)
          ...
          <p class="card-region">{{ $office->city }} / <span class="postcode-uppercase">{{ strtoupper($office->postcode) }}</span></p>
          @if(!empty($office->features) && is_array($office->features))
            <div class="card-features">
              @foreach($office->getExchangeFeatureLabels() as $label)
                <span class="card-feature-badge">{{ $label }}</span>
              @endforeach
            </div>
          @endif
            ...
            <div class="special-rate-banner">
              ...
              <span class="special-rate-values">
                @if($office->special_rate_buy !== null)
                  <span class="special-rate-item buy">خرید {{ farsi_num(number_format($office->special_rate_buy)) }} تومان</span>
                @endif
                @if($office->special_rate_sell !== null)
                  <span class="special-rate-item sell">فروش {{ farsi_num(number_format($office->special_rate_sell)) }} تومان</span>
                @endif
              </span>
            </div>
          @endif
            ...
            <div class="card-rates">
              ...
              <div class="rates-row">
                <div class="rate-item buy">
                  ...
                  <span class="value">{{ farsi_num(number_format($buy)) }} تومان @if($trend['buy'] === 'up')<span class="rate-trend-arrow up" title="نرخ نسبت به قبل بالاتر"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="18 15 12 9 6 15"/></svg></span>@elseif($trend['buy'] === 'down')<span class="rate-trend-arrow down" title="نرخ نسبت به قبل پایین‌تر"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></span>@endif @if($isBestBuy)<span class="best-badge buy">بهترین</span>@endif</span>
                </div>
                <div class="rate-item sell">
                  <span class="label">فروش</span>
                  <span class="value">{{ farsi_num(number_format($sell)) }} تومان @if($trend['sell'] === 'up')<span class="rate-trend-arrow up" title="نرخ نسبت به قبل بالاتر"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="18 15 12 9 6 15"/></svg></span>@elseif($trend['sell'] === 'down')<span class="rate-trend-arrow down" title="نرخ نسبت به قبل پایین‌تر"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></span>@endif @if($isBestSell)<span class="best-badge sell">بهترین</span>@endif</span>
                </div>
              </div>
            </div>
          @endif
          @if(count($office->getAcceptedPaymentMethods()) > 0)
            <div class="card-accepted-payments">
              <span class="card-accepted-label">پرداخت با</span>
              <div class="card-accepted-logos">
                @foreach($office->getAcceptedPaymentMethods() as $pm)
                  @include('partials.payment-method-logo', ['key' => $pm, 'size' => 24, 'class' => 'card-payment-logo'])
                @endforeach
              </div>
            </div>
          @endif
          @if($office->hasTransferFee())
            <p class="card-transfer-fee" style="font-size:0.75rem;color:var(--text-muted);margin-top:0.5rem;padding-top:0.5rem;border-top:1px solid var(--border);">کارمزد حواله زیر {{ farsi_num(number_format($office->transfer_fee_under_amount, 0)) }} پوند: {{ farsi_num(number_format($office->transfer_fee_amount, 0)) }} پوند</p>
          @endif
        </article>
      @endforeach
    </div>
...
@endsection
@section('bottom_nav')
  @include('partials.bottom-nav', ['nav_items' => $nav_items ?? [], 'current_route' => 'exchanges.index'])
@endsection
@push('scripts')
<script>
(function() {
  ...
  var avgBuy = {{ (int) $avgBuy }};
  var avgSell = {{ (int) $avgSell }};
