...
@section('body_start')
  <header class="header" id="header">
    <nav class="nav container">
      <a href="{{ route('exchanges.index') }}" class="logo">{{ $exchangeOffice->name }}</a>
...
@section('content')
  ...
  <main>
      ...
      <div class="container hero-container">
        <div class="hero-content">
          ...
          <div class="hero-header">
            <h1 class="hero-title">{{ $exchangeOffice->hero_title ?? $exchangeOffice->name }}</h1>
            <p class="hero-subtitle">{{ $exchangeOffice->hero_subtitle ?? 'انتقال لحظه‌ای بین ایران و بریتانیا' }}</p>
            <p class="hero-location">{{ $locationLine }}</p>
            @if(count($featureLabels) > 0)
              <div class="hero-money-chips" aria-label="ویژگی‌های صرافی">
                @foreach($featureLabels as $label)
                  <span class="money-chip">{{ $label }}</span>
                @endforeach
              </div>
            @else
              <div class="hero-money-chips" aria-label="ویژگی‌های صرافی">
          ...
          @if($exchangeOffice->landing_show_calculator !== false)
          <div class="calculator-card">
            @if($hasSpecial && ($specialSell !== null || $specialBuy !== null))
              <div class="special-rate">
                ...
                <strong class="special-rate-value">{{ number_format($specialSell ?? $specialBuy) }} تومان</strong>
              </div>
            @endif
            ...
            <div class="calc-footer">
              ...
              <div class="calc-extra">
                @if(count($paymentMethods) > 0)
                  <span class="pay-chip">پرداخت با <strong>{{ strtoupper(implode(', ', $paymentMethods)) }}</strong></span>
                @endif
                @if($exchangeOffice->hasTransferFee())
                  <span class="fee-chip">کارمزد حواله زیر {{ number_format($exchangeOffice->transfer_fee_under_amount) }} پوند: <strong>{{ $exchangeOffice->transfer_fee_amount }} پوند</strong></span>
                @endif
              </div>
            </div>
          </div>
    ...
    @if($exchangeOffice->landing_show_rates !== false && $ratesToIrr->isNotEmpty())
    <section class="rates-strip" aria-label="نرخ لحظه‌ای ارزها">
      <div class="container">
        <div class="rates-grid">
          @foreach($ratesToIrr->take(4) as $rate)
            @php
              $trend = $exchangeOffice->getRateTrend();
              $direction = ($rate->from_currency === 'GBP' ? ($trend['sell'] ?? null) : null);
            @endphp
            <article class="rate-item">
              <span class="rate-pair">{{ $rate->from_currency }} / IRT</span>
              <strong class="rate-value">{{ number_format((float) $rate->sell_rate) }}</strong>
              @if($direction)
                <span class="rate-change {{ $direction }}">{{ $direction === 'up' ? '+0.8%' : '-0.2%' }}</span>
              @endif
            </article>
          @endforeach
        </div>
        ...
        <div class="services-grid">
          @foreach($exchangeOffice->getLandingServices() as $service)
            <article class="service-card">
              ...
              <h3>{{ $service['title'] }}</h3>
              <p>{{ $service['description'] ?: 'خدمات با کیفیت و شفاف.' }}</p>
            </article>
          @endforeach
        </div>
      </div>
      ...
      <div class="container">
        <div class="about-content">
          <div class="about-text">
            <h2>{{ $exchangeOffice->name }}، همکار قابل اعتماد شما</h2>
            @if(filled($exchangeOffice->about))
              <p>{!! nl2br(e($exchangeOffice->about)) !!}</p>
            @else
          ...
          <div class="about-visual">
            <img src="{{ $exchangeOffice->aboutImageUrl() }}" alt="" loading="lazy" width="560" height="560">
          </div>
        </div>
      </div>
    </section>

    @if($exchangeOffice->landing_show_contact !== false)
      ...
      <div class="container">
        ...
        <div class="location-grid">
          <div class="location-info">
            @if($exchangeOffice->getFullAddress())
              <div class="info-item">
                <span class="info-icon">📍</span>
                <div>
                  <h3>آدرس</h3>
                  <p>{{ $exchangeOffice->getFullAddress() }}</p>
                </div>
              </div>
            @endif
            @if(filled($exchangeOffice->phone))
              <div class="info-item">
                <span class="info-icon">📞</span>
                <div>
                  <h3>تلفن</h3>
                  <p><a href="tel:{{ preg_replace('/\s+/', '', $exchangeOffice->phone) }}" dir="ltr" class="landing-cta" data-click-type="call">{{ $exchangeOffice->phone }}</a></p>
                </div>
              </div>
            @endif
            @if(filled($exchangeOffice->email))
              <div class="info-item">
                <span class="info-icon">✉️</span>
                <div>
                  <h3>ایمیل</h3>
                  <p><a href="mailto:{{ $exchangeOffice->email }}">{{ $exchangeOffice->email }}</a></p>
                </div>
              </div>
            @endif
          </div>
          <div class="contact-form-wrap">
            @if(session('contact_success'))
              <p class="contact-form-msg contact-form-msg--success">{{ session('contact_success') }}</p>
            @elseif(session('contact_error'))
              <p class="contact-form-msg contact-form-msg--error">{{ session('contact_error') }}</p>
            @endif
            @if(filled($exchangeOffice->email))
              <form action="{{ route('exchanges.contact', $exchangeOffice) }}" method="post" class="contact-form" id="contact-form">
                @csrf
                <div class="form-group">
                  <label for="contact-name">نام <span class="required">*</span></label>
                  <input type="text" id="contact-name" name="name" value="{{ old('name') }}" required maxlength="255" placeholder="نام شما">
                  @error('name')<span class="form-error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                  <label for="contact-email">ایمیل <span class="required">*</span></label>
                  <input type="email" id="contact-email" name="email" value="{{ old('email') }}" required placeholder="example@email.com">
                  @error('email')<span class="form-error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                  <label for="contact-phone">تلفن</label>
                  <input type="text" id="contact-phone" name="phone" value="{{ old('phone') }}" maxlength="50" placeholder="اختیاری" dir="ltr">
                  @error('phone')<span class="form-error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                  <label for="contact-message">پیام <span class="required">*</span></label>
                  <textarea id="contact-message" name="message" rows="4" required maxlength="2000" placeholder="متن پیام خود را بنویسید...">{{ old('message') }}</textarea>
    ...
    <div class="container">
      <div class="footer-top">
        <div class="footer-brand">
          <a href="{{ route('exchanges.index') }}" class="footer-logo">{{ $exchangeOffice->name }}</a>
          <p class="footer-description">
            {{ $exchangeOffice->tagline ?? 'ارائه‌دهنده خدمات تخصصی انتقال ارز با بهترین نرخ، تسویه سریع و پشتیبانی حرفه‌ای.' }}
          </p>
          @if(in_array('fca_regulated', $exchangeOffice->features ?? []))
        ...
        <div class="footer-col">
          ...
          <ul class="footer-links">
            ...
            <li><a href="{{ route('exchanges.index') }}">بازگشت به دایرکتوری</a></li>
          </ul>
        </div>
        <div class="footer-col">
          <h3 class="footer-title">اطلاعات تماس</h3>
          <ul class="footer-contact">
            <li><span class="footer-contact-label">آدرس:</span> <span>{{ $exchangeOffice->getFullAddress() ?: '—' }}</span></li>
            @if($exchangeOffice->phone)
              <li><span class="footer-contact-label">تلفن:</span> <a href="tel:{{ preg_replace('/\s+/', '', $exchangeOffice->phone) }}" dir="ltr">{{ $exchangeOffice->phone }}</a></li>
            @endif
            @if($exchangeOffice->email)
              <li><span class="footer-contact-label">ایمیل:</span> <a href="mailto:{{ $exchangeOffice->email }}">{{ $exchangeOffice->email }}</a></li>
            @endif
          </ul>
        </div>
        <div class="footer-col">
          ...
          <div class="social-links">
            ...
            @if($whatsappUrl)
              <a href="{{ $whatsappUrl }}" target="_blank" rel="noopener noreferrer" aria-label="واتساپ">
      ...
      <div class="footer-bottom">
        <p class="copyright">© <span id="year"></span> {{ $exchangeOffice->name }}. تمامی حقوق محفوظ است.</p>
        @if($exchangeOffice->fca_number)
  ...
  @if($whatsappUrl)
  <a class="floating-whatsapp" href="{{ $whatsappUrl }}" target="_blank" rel="noopener noreferrer" aria-label="تماس سریع واتساپ" title="تماس سریع واتساپ">
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12.04 2c-5.49 0-9.94 4.45-9.94 9.94 0 1.75.46 3.47 1.33 4.98L2 22l5.23-1.36a9.9 9.9 0 0 0 4.81 1.23h.01c5.49 0 9.95-4.45 9.95-9.94C22 6.45 17.53 2 12.04 2zm5.79 14.05c-.24.69-1.39 1.31-1.92 1.39-.49.07-1.12.1-1.81-.12-.42-.13-.96-.32-1.66-.62-2.92-1.26-4.83-4.2-4.98-4.4-.14-.2-1.19-1.58-1.19-3.01s.75-2.13 1.02-2.42c.26-.29.57-.36.76-.36.19 0 .38 0 .55.01.18.01.42-.07.66.5.24.57.81 1.98.88 2.12.07.14.12.31.02.5-.1.19-.14.31-.29.48-.14.17-.3.38-.42.5-.14.14-.28.29-.12.57.17.29.73 1.2 1.56 1.94 1.07.95 1.97 1.24 2.25 1.38.29.14.45.12.62-.07.17-.19.72-.84.91-1.13.19-.29.38-.24.64-.14.26.1 1.67.79 1.95.94.29.14.48.21.55.33.07.12.07.67-.17 1.35z"/></svg>
    <span>تماس سریع</span>
  </a>
  @endif
  ...
  <script>
    (function () {
      ...
      var BUY_RATE = {{ $buyRate }};
      var SELL_RATE = {{ $sellRateVal }};
